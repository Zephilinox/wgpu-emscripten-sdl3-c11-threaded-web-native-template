...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
    printf("[AppInit] Example 01: Minimal WebGPU + SDL3 Triangle...\n");
    ...
    *appstate = app;
    ...
    app->window = SDL_CreateWindow("Example 01: WebGPU Triangle", app->width, app->height, SDL_WINDOW_RESIZABLE);
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ...
    if (!app->pipelinesInitialized) {
        ...
        WGPUBufferDescriptor triDesc = { .label = make_str_view("Vertex Buffer"), .usage = WGPUBufferUsage_Vertex | WGPUBufferUsage_CopyDst, .size = sizeof(triangleVertices) };
        app->triangleVertexBuffer = wgpuDeviceCreateBuffer(app->device, &triDesc);
        wgpuQueueWriteBuffer(app->queue, app->triangleVertexBuffer, 0, triangleVertices, sizeof(triangleVertices));
        ...
        printf("[Init] Example 01 initialized.\n");
    }
...
void SDL_AppQuit(void *appstate, SDL_AppResult result) {
    if (appstate) {
        AppState* app = (AppState*)appstate;
        printf("[AppQuit] Shutting down Example 01.\n");
