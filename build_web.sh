...
echo "  - Example 05:        http://localhost:8000/05_compute_threads.html"

# ============================================================================
# itch.io bundle: standalone zip for Example 05 (compute + threads).
# Built against a minimal shell (no coi-serviceworker, no nav, no sliders).
# itch.io provides the cross-origin isolation required for pthreads; the page
# is emitted as index.html so it is itch.io's required entry point.
# ============================================================================
ITCH_DIR="$ROOT_DIR/bin/itch"
ITCH_EX=05_compute_threads
ITCH_STAGE="$ITCH_DIR/$ITCH_EX"
rm -rf "$ITCH_STAGE"
mkdir -p "$ITCH_STAGE"

echo "Building itch.io bundle: $ITCH_EX (index.html)..."
ITCH_FLAGS="--use-port=emdawnwebgpu -s USE_SDL=3 -O2 -s WASM=1 -s ALLOW_MEMORY_GROWTH=1 --shell-file $ROOT_DIR/shell_itch.html -I$DEPS_DIR/sdl3/include -pthread -s PTHREAD_POOL_SIZE=2"
emcc "$ROOT_DIR/$ITCH_EX.c" -o "$ITCH_STAGE/index.html" $ITCH_FLAGS

if command -v zip > /dev/null 2>&1; then
    (cd "$ITCH_STAGE" && zip -rq "$ITCH_DIR/$ITCH_EX.zip" .)
    echo "itch.io bundle created: $ITCH_DIR/$ITCH_EX.zip"
    echo "  Upload it at https://itch.io/game/new (Kind of game: HTML, embed as ZIP)."
else
    echo "NOTE: 'zip' not found; staged itch bundle is ready to zip manually at:"
